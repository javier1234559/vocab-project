...
import { ICreateVocabDTO } from "~/types";
import ItemVocab from "~/components/Form/LessonForm/ItemVocab";
...
function ListVocab({ listVocab, setlistVocab }: Props) {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id && listVocab !== null) {
      setlistVocab((prevlistVocab: ICreateVocabDTO[]) => {
        ...
        const oldIndex = clonelistItem.findIndex(
          (item) => item.index === active.id
        );
        const newIndex = clonelistItem.findIndex(
  ...
  const handleOnChange = (id: number, event: any) => {
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        className="group mb-8 cursor-pointer flex w-full py-10 items-center rounded-md bg-secondary"
        onClick={() =>
          listVocab &&
          listVocab.length > 0 &&
          addItems(listVocab[listVocab.length - 1].index)
        }
      >
        <div className="flex items-center justify-center w-full">
          <span className="relative group-hover:text-accent transition uppercase whitespace-nowrap font-bold">
            + THÊM THẺ
            <div className="transition absolute w-full h-1 mt-2 group-hover:bg-accent" />
          </span>
        </div>
      </button>
    </DndContext>
